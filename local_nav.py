...
def simple_navigation():
    ...
    if state == "global":
        ...
        if not next_checkpoints:
            log("All checkpoints reached. Stopping.")
            is_goal_reached = True
            motor_left_target = 0
            motor_right_target = 0
            return

        # If just got back to global navigation, set the motor speed to the default speed
        if ((motor_left_target !=  initial_speed) or  (motor_right_target != initial_speed)) and not is_goal_reached:
            motor_left_target = initial_speed
            motor_right_target = initial_speed
            log("Setting motor speed to default speed")

        # Check if the robot has reached the next checkpoint
        if is_checkpoint_reached(current_position, next_checkpoints[0]):
            log("Checkpoint {} reached".format(next_checkpoints[0]))
            next_checkpoints.pop(0)
        else:
            # TODO: move the robot to the next checkpoint
            log("Moving to checkpoint {}".format(next_checkpoints[0]))


    elif state == "local":
        y = [0, 0]
        # Calculate the motor speed correction to avoid obstacles
        for i in range(len(obst)):
            y[0] += w_l[i] * obst[i]
            y[1] += w_r[i] * obst[i]
